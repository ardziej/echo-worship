...
tempdir="${destinationpath}/snippets"
listfile="${destinationpath}/list.txt"
...
for i in $(seq 1 $desiredsnippets)
  do
    ...
    ffmpeg -i $sourcefile -ss $formattedstart -vf scale=$dimensions -vframes 1 $destinationpath/$i.jpg
...
ffmpeg -f concat -safe 0 -i $listfile -c copy $destinationpath/video.$extension
